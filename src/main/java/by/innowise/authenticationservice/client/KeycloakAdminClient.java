...
public class KeycloakAdminClient {
    ...
    public String createUser(
            KeycloakUserCreateRequest request,
            String password
    ) {
        String accessToken =
                tokenClient.createServiceAccessToken();

        String keycloakUserId =
                createUserRecord(accessToken, request);
    ...
    private String createUserRecord(
            String accessToken,
            KeycloakUserCreateRequest request
    ) {
        try {
    ...
    private void setPassword(
        ...
        KeycloakCredentialRequest credential =
        ...
        try {
            restClient
                    .put()
                    .uri(
                            properties.adminUserPasswordUrl(
                                    keycloakUserId
                            )
                    )
                    .headers(headers ->
            ...
            throw new IdentityProviderException(
                    "Failed to set Keycloak user password",
                    exception
            );
        }
    }
    ...
    private void assignRealmRole(
        ...
        KeycloakRoleResponse role =
                getRealmRole(accessToken, roleName);

        try {
            restClient
                    .post()
                    .uri(
                            properties.adminUserRealmRolesUrl(
                                    keycloakUserId
                            )
                    )
                    .headers(headers ->
            ...
            throw new IdentityProviderException(
                    "Failed to assign Keycloak realm role",
                    exception
            );
        }
    }

    private KeycloakRoleResponse getRealmRole(
            String accessToken,
            String roleName
    ) {
        try {
            KeycloakRoleResponse role = restClient
                    .get()
                    .uri(
                            properties.adminRealmRoleUrl(
                                    roleName
                            )
                    )
                    .headers(headers ->
            ...
            throw new IdentityProviderException(
                    "Failed to obtain Keycloak realm role",
                    exception
            );
        } catch (RestClientException exception) {
            throw new IdentityProviderException(
    ...
    private void deleteUserSilently(
            String accessToken,
            String keycloakUserId
    ) {
        try {
